...
if(isset($_POST["submit"])){
  ...
  if($result){
    echo "only price is added not city yet....<br />";
      ...
      <div class="card">
        ...
        <div class="card-content center">
          ...
          <form method="POST" action="">
            ...
            <a id="selectAll" onclick="selectAll()" class="waves-effect waves-light btn-small">Select All</a>
            <a id="clearAll" onclick="clearAll()" class="waves-effect waves-light btn-small">Clear All</a>
            <a id="closeAll" onclick="closeAll()" class="waves-effect waves-light btn-small">Close All</a>
            <p>
              <label>
                <input type="checkbox" id="one" onclick="funOne()"/>
                <span>821101</span>
              </label>
            </p>
            <p>
              <label>
                <input type="checkbox" id="two" onclick="funTwo()"/>
                <span>821102</span>
              </label>
            </p>
            <p>
              <label>
                <input type="checkbox" id="three" onclick="funThree()"/>
                <span>821109</span>
              </label>
            </p>
              ...
              <?php
                ...
                if($result->num_rows > 0){
                  while($row = $result->fetch_assoc()) {
                    echo '<div style="margin-right:50px;display:inline;float:left;"><label><input type="checkbox" name="chk_cities[]" id="'. $row["city_name"].' checkbox-one" value="'.$row["id"].'" /><span>'.$row["id"].' '. $row["city_name"] .' ('. $row["distance"] .' KM away)'.'</span></label></div>';
                  }
                }
               ?>
            </div>
            <div id="citytwo" style="display:none;">
              <?php 
                ...
                if($result->num_rows > 0){
                  while($row = $result->fetch_assoc()){
                    echo '<div style="margin-right:50px;display:inline;float:left;"><label><input type="checkbox" name="chk_cities[]" id="'. $row["city_name"].' checkbox-two" value="'.$row["id"].'" /><span>'.$row["id"].' '. $row["city_name"] .' ('. $row["distance"] . ' KM away)' .'</span></label></div>';
                  }
                }
              ?>
            </div>
            <div id="citythree" style="display:none;">
              <?php 
                ...
                if($result->num_rows > 0){
                  while($row = $result->fetch_assoc()){
                    echo '<div style="margin-right:50px;display:inline;float:left;"><label><input type="checkbox" name="chk_cities[]" id="'. $row["city_name"].' checkbox-three" value="'.$row["id"].'" /><span>'.$row["id"].' '. $row["city_name"] .' ('. $row["distance"] .' KM away)'.'</span></label></div>';
                  }
                }
              ?>
            </div>
