  ...
  <?php foreach($added_products as $added_product){ ?>
   ...
   <!--this is old code-->
    ...
    <li class="collection-item avatar">
      ...
      <span class="title"><strong><?=$added_product->product_name?></strong></span>
      <p><?php if((int)$added_product->offer_price != 0){ $total[] = $added_product->offer_price; ?> 
        <span style="font-size:125%;" class="green-text" class="fa fa-inr" ><?=$added_product->offer_price?> 
      </span><strike class="fa fa-inr" ><?=$added_product->price?></strike> 
      <span data-badge-caption class="new badge left" > <?=substr($percent,0,4)?>% Off</span> <?php } 
      else { $total[] = $added_product->price; ?><i class="fa fa-inr"></i><?=$added_product->price?> <?php } ?>  <br>
      </p>
    </li>
